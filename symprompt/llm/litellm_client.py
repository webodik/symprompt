...
def _strip_markdown_code_blocks(text: str) -> str:
    """
    Remove markdown code fences from LLM output to recover raw content.
    """
    if not text:
        return text
...
class LiteLLMLLM(LLMInterface):
    """
    LLMInterface implementation that calls providers via the LiteLLM library.
    """
        ...
        for attempt in range(retries + 1):
            try:
                ...
                if attempt < retries:
                    ...
                    logger.error("LiteLLM timeout after %s attempts", retries + 1)
                    raise
