...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run profile evolution.")
    parser.add_argument(
        "--iterations", "-n",
        type=int,
        default=None,
    ...
    if not model_name:
        raise RuntimeError(
            ...
            "before running profile evolution."
        )
    ...
    output_dir = root / "evolution" / "openevolve_output_profiles"
    ...
    if backup_path is not None:
        print(f"Created profiles evolution DB backup at {backup_path}")

    initial_program = root / "symprompt" / "symil" / "profiles.py"
    evaluator = root / "symprompt" / "evolution" / "eval_profiles.py"
    ...
    print("Profiles evolution finished.")
